...
import { SERVER_DIRECTORY, PAGES_MANIFEST } from '../../shared/lib/constants'
...
export async function isPagesPageStatic(context: {
  ...
  if (isEdgeRuntime(context.pageRuntime) && context.edgeInfo) {
    const runtime = await getRuntimeContext({
      paths: context.edgeInfo.files.map((file: string) =>
        path.join(context.distDir, file)
      ),
      env: context.edgeInfo.env,
      edgeFunctionEntry: {
        ...context.edgeInfo,
        wasm: (context.edgeInfo.wasm ?? []).map((binding: AssetBinding) => ({
          ...binding,
          filePath: path.join(context.distDir, binding.filePath),
        })),
      },
      name: context.edgeInfo.name,
      useCache: true,
      distDir: context.distDir,
    })

    console.log('EDGE-DEBUG', Object.keys(runtime.context._ENTRIES))
    console.log(
      'EDGE-DEBUG:KEYS',
      Object.keys(
        runtime.context._ENTRIES?.[`middleware_${context.edgeInfo.name}`]
      )
    )
    console.log(
      'EDGE-DEBUG:KEYS:ComponentMod',
      Object.keys(
        runtime.context._ENTRIES?.[`middleware_${context.edgeInfo.name}`]
          .ComponentMod
      )
    ...
    throw new Error("EDGE-DEBUG: don't know what to do here yet")
  }

  // Load the pages manifest so we can reference the module for the page.
  const manifest: PagesManifest = require(path.join(
    context.distDir,
    SERVER_DIRECTORY,
    PAGES_MANIFEST
  ))

  // Lookup the page in the manifest.
  const page = manifest[context.page]
  if (!page) {
    throw new Error(`Invariant: page wasn't found in manifest: ${context.page}`)
  }

  // Try to load the module from the filesystem.
  const module = await RouteModuleLoader.load<PagesRouteModule>(
    path.join(context.distDir, SERVER_DIRECTORY, page)
  )
  ...
  return {
    ...
    isNextImageImported,
    // FIXME: (wyattjoh) find out where these are included
